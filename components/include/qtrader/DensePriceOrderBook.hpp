...
namespace wthft
{
	...
	public:
		...
		{
			if (key == kEmpty || key == std::numeric_limits<uint64_t>::max()) return invalid_index();
			std::size_t slot = hash(key) & (_capacity - 1);
			for (std::size_t probe = 0; probe < _capacity; ++probe)
			{
		...
		bool erase(uint64_t key)
		{
			...
			for (std::size_t probe = 0; probe < _capacity; ++probe)
			{
				if (_entries[slot].key == kEmpty) return false;
	...
	private:
		...
		void backshift_from(std::size_t hole)
		{
			const std::size_t mask = _capacity - 1;
			std::size_t scan = (hole + 1) & mask;
			while (_entries[scan].key != kEmpty)
			{
				const std::size_t home = hash(_entries[scan].key) & mask;
				const std::size_t scan_distance = (scan - home) & mask;
				const std::size_t hole_distance = (hole - home) & mask;
				if (hole_distance < scan_distance)
				{
					_entries[hole] = _entries[scan];
					hole = scan;
				}
				scan = (scan + 1) & mask;
			}
	...
	public:
		...
		// Direct market-data operations. Unlike submit(), these apply an exchange
		// feed's already-matched state transitions and never perform local matching.
		bool feed_add(uint64_t id, Side side, int64_t price, uint64_t quantity)
		{
			uint32_t level = kInvalid;
			if (id == 0 || id == std::numeric_limits<uint64_t>::max() || quantity == 0 ||
				_index.find(id) != kInvalid || !price_index(price, level)) return false;
			const uint32_t index = allocate();
			if (index == kInvalid || !_index.insert(id, index))
			{
		...
		bool feed_reduce(uint64_t id, uint64_t quantity)
		{
			const uint32_t index = _index.find(id);
			if (index == kInvalid || quantity == 0 || quantity > _nodes[index].remaining)
				return false;
			Node& order = _nodes[index];
			PriceLevel& level = order.side == Side::Buy ? _bids[order.level] : _asks[order.level];
			order.remaining -= quantity;
			level.total_quantity -= quantity;
			if (order.remaining == 0)
			{
				// Feed reductions may target any order at a price level, not only the
				// FIFO head. remaining is already zero, so unlink() only repairs links.
				unlink(index);
				release(index);
			}
			return true;
		...
		bool feed_replace(uint64_t old_id, uint64_t new_id, int64_t price, uint64_t quantity)
		{
			const uint32_t index = _index.find(old_id);
			uint32_t level = kInvalid;
			if (index == kInvalid || new_id == 0 ||
				new_id == std::numeric_limits<uint64_t>::max() || quantity == 0 ||
				_index.find(new_id) != kInvalid || !price_index(price, level)) return false;
			// Insert the new key first, so a theoretically full index cannot destroy
			// the old mapping. Both keys point to the same node only during this call.
			if (!_index.insert(new_id, index)) return false;
			Node& order = _nodes[index];
			unlink(index);
			_index.erase(old_id);
			order.id = new_id;
		...
		uint64_t remaining(uint64_t id) const
		{
			const uint32_t index = _index.find(id);
			return index == kInvalid ? 0 : _nodes[index].remaining;
		}
		uint64_t level_quantity(Side side, int64_t price) const
		{
			uint32_t level = kInvalid;
			if (!price_index(price, level)) return 0;
			return side == Side::Buy ? _bids[level].total_quantity : _asks[level].total_quantity;
		}
		const void* node_storage() const { return _nodes.data(); }
		// Read-only FIFO traversal for downstream depth consumers and audits.
		template<typename Visitor>
		void visit_level(Side side, int64_t price, Visitor&& visitor) const
		{
			uint32_t level = kInvalid;
			if (!price_index(price, level)) return;
			const auto& slot = side == Side::Buy ? _bids[level] : _asks[level];
			for (auto i = slot.head; i != kInvalid; i = _nodes[i].next)
				visitor(_nodes[i].id, _nodes[i].remaining);
		}
