...
namespace
{
	...
	public:
		...
		{
#ifndef _MSC_VER
			char pattern[] = "/tmp/qtrader-level2-XXXXXX";
			_fd = ::mkstemp(pattern);
			_path = pattern;
...
int main(int argc, char** argv)
{
	...
	std::thread consumer([&]() {
		...
		for (std::size_t received = 0; received < source.size();)
		{
			...
			if ((received & 1023) == 1023 && book.best_ask() != 0)
			{
				...
				if (!book.submit({strategy_id, wthft::Side::Buy, wthft::OrderType::Market, 0, 1}, sink))
					failed.store(true, std::memory_order_relaxed);
				++simulated_orders;
			}
	...
	if (input_path == nullptr &&
		(simulated_orders != source.size() / 1024 ||
		 sink.volume != simulated_orders || sink.trades != simulated_orders))
		failed.store(true, std::memory_order_relaxed);
	if (completed.load() != source.size() || shm_consumed.load() != source.size())
		failed.store(true, std::memory_order_relaxed);
	std::printf("validation=%s callbacks=%llu simulated_orders=%llu\n",
		failed.load() ? "FAIL" : "PASS",
		static_cast<unsigned long long>(completed.load()),
		static_cast<unsigned long long>(simulated_orders));
